...
$$Operative\;Temperature = - Intercept + (2 - Slope)\cdot IR\; Temperature^*$$
If we implement the formula

```{r}
# estimate op_temp after correction
correction_data$op_temp_corr <- -12.11754 + (2 - 0.68946)*correction_data$ir_temp_corr
```
...
Implementing the `correct_flights_data` function is straightforward. Say that the user has a flights `tibble` called `flights_datax` and an OTM splines complex `tibble` called `otm_splinesx`, to correct their flights data the user would simply need to run the following code. 

```{r, eval = FALSE}
corrected_flights_datax <- correct_flights_data(flights_data = flights_datax, otm_splines = otm_splinesx)
```
