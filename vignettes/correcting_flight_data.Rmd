...
```{r, echo = FALSE}
corr_data %>% filter(gcps == "Y") %>% dplyr::select(!gcps) %>% as_tibble()
```
With this data we can take a first look at how does the reflected temperature measurements made by the drone (`ref_temp`) relate to those collected by the OTMs (`op_temp`).

```{r, fig.align='center', fig.height=4, fig.width=6, echo = FALSE, message=FALSE}
corr_data %>% filter(gcps == "Y") %>% 
  ggplot(aes(x = op_temp, y = ref_temp)) +
