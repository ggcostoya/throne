...
1.  `coverage_per`, a numeric value between 0 - 1 indicating the minimum coverage that a tile should have across all flights provided in order to be included in the matching process. The need for this input can be easily visualized by overlapping the area covered by multiple flights in the provided example `flights_data` (see image below). Due to the unique conditions during each of the `34` flights we conducted for the example data set of this package (e.g., wind etc.), the area covered by each flight was variable, specially when focusing around the edges of our study area. Nonetheless, we foresaw this effect which is why **we ensured that the area covered by each flight would be much larger than the area of interest**, a precaution we recommend all users to follow. Our goal was that the area where OTMs were deployed would be covered by flights and, as seen above, all OTMs deployed on the ground (black dots) fell on areas that were covered by the vast majority of our flights. **As a general guideline, we recommend setting `coverage_per >= 0.9`**, the value the function will default to if `coverage_per` is not specified. Nonetheless, the function output will also provide a `coverage` column for each tile to further inform the users. 
...
By default the `match_data` function will set `error_max = 100`. This is a sufficiently large value to ensure that all tiles are match to an OTM. The purpose of this is to be fully transparent with the users and allowing them to decide post-hoc what threshold to use. As an example, below we show the error map but with `error_max = 100` instead of 5. 
...
As seen above, the `error` never exceeds 10 C. This is a good indication that the OTMs are well suited to describe the thermal dynamics of the area of interest. Below is the distribution of error to further illustrate this point.
...
matches_100  %>%
  ...
  xlab("Matching error (C)") + ylab("Density")
```
...
> **NOTE**: The `tibble` presented above was generated considering an `error_max = 5`. The `throne` package also includes another matches data set generated with `error_max = 100` that can be accessed by calling `matches_100`. Both data sets were generated by setting `coverage_per = 0.9`, the default setting. 
...
# define nrow of matches_5
nrow_matches_5_assigned <- nrow(matches_5[!is.na(matches_5$otm_id),])

matches_5 %>% 
  filter(!is.na(otm_id)) %>%
  group_by(otm_id) %>% 
  summarise(tiles_covered = (n()/nrow_matches_5_assigned)*100) %>% 
  ungroup() %>%
...
merge(matches_100, otms_splines_orient, by = c("otm_id"))%>% 
  ggplot(aes(x = longitude, y = latitude)) +
...
> **NOTE**: In the plot above we use the a `matches` tibble with `error_max = 100` for visualization purposes only.
...
```{r}
# obtain prediction
prediction_237 <- predict_thermal_landscape(matches = matches_100, otm_splines = otms_splines, 
