...
The goal of this vignette is to illustrate the first step of the workflow presented in `throne` the acquisition of thermal photogrammetry data. To do this, you will need to 1) acquire a drone with a mounted IR thermal imaging camera, 2) fly the drone to collect thermal images, and 3) process the thermal images using a photogrammetry software to obtain a thermal orthomosaic of your flight (i.e., a `.tif` format file). Here we provide instructions on how to perform each of these steps and we include links to recommended drones, software and tutorials to complement them. 

# Acquiring a thermal imaging drone

{Additional details on types of drones etc.}

We used a DJI Matrice 200 V2 quadcopter with a mounted Zenmuse XT2 IR thermal imaging camera. 

An important note to consider when acquiring a drone, is the resolution of the mounted thermal imaging camera. IR cameras usually have a much lower  resolution and field of view than visual cameras (i.e., RGB). This can lead to a poor overlap between the images collected via both sources which can be problematic later on. To avoid this issue, we recommend 


so a high vertical and horizontal photo overlap ratio is recommended for the mission, as well as a relatively high flight altitude (e.g. a minimum of 80%/80% and a mission altitude of 300 ft/90 m above ground level). This, of course, depends on the spatial scale of the studied landscape and the desired resolution thereof. Regardless, nake sure you're mission is built around the _thermal_ camera's field of view rather than the visual camera - especially if you're flying in a rugged landscape - otherwise you risk running into stitching issues when processing the photos! It's generally a good idea to extend the flight mission area to exceed the study site's boundaries by a few meters to compensate for this difference.

(*insert ThermalOverlay.png here* Caption: The field of view is typically smaller for thermal cameras compared to visual cameras. This overlay - while not perfect - aims to show the difference between a visual and thermal photo taken right after one another, to illustrate the difference in the area covered.)
...
(*insert gcp_nadir.png here* Caption: If you use ground control points for your missions, make sure that they are visible in all your flights.)
...
(*insert Pix4D.gif here* Caption: How to create and start processing a project)
...
(*insert gcp_example.png here*. Caption: Ground control points (GCPs) will show as blue/green markers in the rayCloud editor once they have been loaded and optimized with the point cloud.)
